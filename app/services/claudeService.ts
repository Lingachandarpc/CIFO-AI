...
export async function generateNarrativeWithWebSearch(
  ...
  selectedModel?: string
      ...
      const resolvedModel = (() => {
        ...
        if (requested === 'claude-3-opus-20240229' || requested === 'claude-opus') return 'claude-3-opus-20240229';
        if (requested === 'claude-3-haiku-20240307' || requested === 'claude-haiku') return 'claude-3-haiku-20240307';
        if (requested === 'claude-sonnet-4-20250514' || requested === 'claude-sonnet') return 'claude-sonnet-4-20250514';
        return MODEL;
      })();
...
- For Practical style: include references from books, research papers, journals, or credible industry reports when available from web context. If not available, clearly state assumptions and do not fabricate citations.
...
${enableWebSearch && searchContext ? '\nYou have been provided with current web search results. ALWAYS prioritize this information over your training data when there are discrepancies. If the query asks about current/recent information, use ONLY the web results provided.' : ''}`;
    ...
    return { 
      ...
      modelUsed: selectedModel || 'claude-sonnet' 
    };
  }
}
