...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognitionCtor) {
      ...
      recognitionRef.current.onstart = () => {
        setIsListening(true);
        // If narration audio is playing, stop it when the user starts speaking
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        // Stop any playing audio as soon as we capture speech
        if (currentSourceRef.current) {
          try { currentSourceRef.current.stop(); } catch {};
          try { currentSourceRef.current.disconnect(); } catch {}
          currentSourceRef.current = null;
      ...
      try {
        ...
        if (authData.authenticated && authData.user?.id) {
          ...
          if (historyData.success && historyData.history) {
            // Optionally load historical messages
            console.log('Chat history loaded:', historyData.history.length, 'messages');
          }
        } else {
          setIsAuthenticated(false);
          // Load local history if not authenticated
  ...
  const saveToHistory = (query: string, mode: SearchMode) => {
    ...
    if (isAuthenticated) {
      ...
      }).catch((error) => console.error('Error saving to chat history:', error));
    }
  };
  ...
  const handlePlayAudio = async (base64: string) => {
    ...
    // Start talkback (speech recognition) so user can interject — onstart/onresult will stop audio
    try {
      recognitionRef.current?.start?.();
    } catch (err) {
      // ignore if mic permissions unavailable or recognition already running
    }
  };
      ...
      if (isAuthenticated) {
        fetch('/api/chronoread/chat', {
          ...
          body: JSON.stringify({
            role: 'assistant',
            content: narrativeText,
            mode: currentMode === SearchMode.BOOK ? 'BOOK' : currentMode === SearchMode.CASE_STUDY ? 'CASE_STUDY' : 'ASK',
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full py-4 px-6 bg-gradient-to-r from-lime-400 to-lime-500 text-black font-bold rounded-xl hover:from-lime-300 hover:to-lime-400 transition-all transform hover:scale-105 active:scale-95"
            >
              Sign In
            </button>
            <button
          ...
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSubmit} className="relative group">
              ...
              <input
                ...
                className="w-full bg-neutral-900/50 border border-neutral-800 rounded-2xl py-4 pl-36 md:pl-48 pr-28 focus:outline-none focus:border-neutral-600 focus:bg-neutral-900 transition-all text-sm md:text-base placeholder-neutral-600"
              />
